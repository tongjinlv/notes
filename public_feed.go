...
// parseUsersDirNotePath 解析 users 下相对路径（不含 note.md），得到 provider、login、笔记目录段（YYYYMM/id、YYYY/MM/id、YYYY/MM/DD/id）。
func parseUsersDirNotePath(dirRel string) (provider, login string, noteParts []string, ok bool) {
	...
	if len(parts) == 5 && (parts[0] == "github" || parts[0] == "gitee") {
		noteParts = parts[2:5]
		if isNoteLayoutDir(noteParts) {
			return parts[0], parts[1], noteParts, true
		}
...
func collectPublicPosts(vaultBase, vaultPassphrase string) ([]PublicPostItem, error) {
	...
	err = filepath.WalkDir(usersDir, func(path string, d fs.DirEntry, err error) error {
		...
		note, e := parseNoteMD(raw, noteLayoutLeafID(noteParts), mt, vaultPassphrase)
...
func loadPublicPostDetail(vaultBase, vaultPassphrase, provider, login, dirRel string) (PublicPostItem, error) {
	...
	note, err := parseNoteMD(raw, noteLayoutLeafID(parts), info.ModTime(), vaultPassphrase)
...
func registerPublicAPI(r *gin.Engine, vaultBase string, vaultPassphrase string) {
	...
	r.GET("/api/public/asset/:provider/:login/*filepath", func(c *gin.Context) {
		...
		note, err := parseNoteMD(raw, leafID, info.ModTime(), vaultPassphrase)
		if err != nil || !note.Public {
			c.Status(http.StatusNotFound)
			return
		}
