...
- **笔记与图片加密（可选）**：在 `notes-config.json` 中设置 `vaultPassphrase`，或使用环境变量 `NOTES_VAULT_PASSPHRASE`（优先）。启用后写入磁盘的 `note.md` 与粘贴上传的图片（`image-*.*`）均为 AES-256-GCM 密文，推送到 GitHub 后若无口令难以还原。**请勿把口令与仓库一起提交**；口令丢失将无法解密已有数据。侧栏顺序等 JSON 仍明文；仅本机/服务进程可解密后通过浏览器展示。
